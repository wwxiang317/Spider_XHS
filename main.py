...
class Data_Spider():
    ...
    def spider_user_all_note(self, user_url: str, cookies_str: str, base_path: dict, save_choice: str, excel_name: str = '', proxies=None):
        ...
        try:
            ...
            if save_choice == 'all' or save_choice == 'excel':
                excel_name = 'all_notes_' + user_url.split('/')[-1].split('?')[0]
    ...
    def spider_user_latest_collect_note(self, user_url: str, cookies_str: str, base_path: dict, save_choice: str, count=1, proxies=None):
        """
        爬取用户的最新收藏笔记
        :param user_url:
        :param cookies_str:
        ...
        page_note_count = 10
        try:
            success, msg, notes = self.xhs_apis.get_user_latest_collect_note_info(user_url, cookies_str, 1 + count // page_note_count, proxies)
            if success:
                count = min(count, len(notes))
                logger.info(f'爬取 {user_url} 收藏笔记数量: {count}')
                for i in range(count):
                    note = notes[i]
                    note_url = f"https://www.xiaohongshu.com/explore/{note['note_id']}?xsec_token={note['xsec_token']}"
                    note_list.append(note_url)
            if save_choice == 'all' or save_choice == 'excel':
                excel_name = 'latest_collects_' + user_url.split('/')[-1].split('?')[0]
            self.spider_some_note(note_list, cookies_str, base_path, save_choice, excel_name, proxies)
        except Exception as e:
            success = False
            msg = e
    ...
    def test_spider_user_info(self, user_id, cookies_str):
        success, msg, res_json = self.xhs_apis.get_user_info(user_id, cookies_str)
        #logger.info(f'获取用户信息 {user_id}: {success}, msg: {msg}, res_json: {json.dumps(res_json, ensure_ascii=False)}')
        logger.info(f'获取用户信息 {user_id}: {success}, msg: {msg}')
...
if __name__ == '__main__':
    ...
    # user_url = 'https://www.xiaohongshu.com/user/profile/683fe17f0000000023017c6a?xsec_token=AB-GhAToFu07JwNk_AMICHnp7bSTjVz2beVIDBwSyPwvM=&xsec_source=pc_feed'
    ...
    # 4 测试获取用户信息
    # data_spider.test_spider_user_info(user_id, cookies_str)

    ####################################从这里开始！！！！！！！！#########################################
    # 5 爬取用户最新收藏的笔记
    # 用法：
    #     python main.py 2
    # 2为你想爬取的笔记数量，略去默认爬取最新笔记
    user_url = 'https://www.xiaohongshu.com/user/profile/683fe17f0000000023017c6a?xsec_token=AB-GhAToFu07JwNk_AMICHnp7bSTjVz2beVIDBwSyPwvM=&xsec_source=pc_feed'
    ...
    if (len(sys.argv) > 1):
        ...
        count = 1
    data_spider.spider_user_latest_collect_note(user_url, cookies_str, base_path, 'all', count)
    #####################################################################################################
